...
import { LoaderIcon, Check, Copy, ArrowRight, GitCompareArrows, Play, Code2, Wrench, Activity, RefreshCw, X as XIcon, File } from "lucide-react"
...
export function WebhooksTester() {
      ...
      try {
        ...
        const result: DeliveryResult = {
          ...
          requestHeaders: {
            ...
            ...(useSignature ? { "X-Webhook-Signature": `sha256=${Math.random().toString(36).substring(2, 15)}` } : {})
          },
          requestBody: payload,
        ...
        if (isSuccess) {
          ...
          result.error = "O servidor retornou um erro 500 Internal Server Error"
        }
        ...
        const result: DeliveryResult = {
          ...
          requestBody: payload,
          error: error instanceof Error ? error.message : "Ocorreu um erro ao processar a requisição"
        }
  ...
  const generateCodeSample = (language: string) => {
    const payloadObj = payload ? JSON.parse(payload) : { type: "event.type", data: {} }
...
print(f'Resposta: {response.text}')`
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-1">Teste e Depuração de Webhooks</h2>
        <p className="text-muted-foreground">Ferramentas para testar e validar seus webhooks antes da implementação em produção</p>
      </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Configure as opções para testar seu webhook</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="auth-header">Cabeçalho de Autenticação (opcional)</Label>
                  <Input
                    id="auth-header"
                ...
                <div className="flex items-center space-x-2 pt-2">
                  ...
                  />
                  <Label htmlFor="use-signature">Incluir assinatura do webhook</Label>
                </div>
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <Select value={selectedTemplate} onValueChange={handleTemplateChange}>
                    ...
                    <SelectContent>
                      <SelectItem value="">Evento personalizado</SelectItem>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {deliveryResult 
                      ? `Enviado em ${new Date(deliveryResult.timestamp).toLocaleString()}` 
                ...
                <CardContent>
                  {deliveryResult ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        {deliveryResult.statusCode && (
                          <Badge variant="outline" className={cn(
                            "font-mono",
                            deliveryResult.statusCode >= 200 && deliveryResult.statusCode < 300
                      ...
                      <Tabs defaultValue="request">
                          ...
                          <div>
                            ...
                            <div className="bg-muted p-2 rounded max-h-32 overflow-auto">
                              <pre className="text-xs">
                                {JSON.stringify(deliveryResult.requestHeaders, null, 2)}
                              </pre>
                            </div>
                          </div>
                        ...
                        <TabsContent value="response" className="space-y-4 mt-2">
                          {deliveryResult.status === "success" || (deliveryResult.status === "failure" && deliveryResult.responseBody) ? (
                            <>
                              <div>
                                <h3 className="text-sm font-medium mb-1">Headers:</h3>
                                <div className="bg-muted p-2 rounded max-h-32 overflow-auto">
                                  <pre className="text-xs">
                                    {JSON.stringify(deliveryResult.responseHeaders, null, 2)}
                                  </pre>
                                </div>
                              </div>
                            ...
                            <div className="py-12 text-center">
                              <p className="text-muted-foreground">
                                {deliveryResult.error || "Nenhuma resposta recebida"}
                              </p>
                            </div>
                          )}
                          ...
                          {deliveryResult.error && (
                            <div>
                              <h3 className="text-sm font-medium mb-1 text-red-500">Erro:</h3>
                              <div className="bg-red-50 text-red-700 p-2 rounded">
                                <p className="text-xs">{deliveryResult.error}</p>
                              </div>
                            </div>
                          )}
                        </TabsContent>
                    ...
                    <div className="text-center py-16">
                      ...
                      <h3 className="text-lg font-medium">Nenhum teste enviado</h3>
                      <p className="text-muted-foreground mt-1">
                        Configure e envie um webhook de teste para ver os resultados
                      </p>
                    </div>
                  )}
                </CardContent>
                      ...
                      <div className="divide-y">
                        {eventHistory.map((result) => (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              <span className="text-xs truncate max-w-[180px]" title={result.url}>
                                {result.url}
                              </span>
        ...
        <TabsContent value="sandbox" className="space-y-6 pt-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Ambiente simulado para testar seus webhooks sem precisar configurar um servidor
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="bg-muted/50 p-4 rounded-lg border border-dashed">
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 bg-background p-2 rounded text-sm font-mono">
                      https://sandbox.metricassaas.com/webhooks/test-{Math.random().toString(36).substring(2, 10)}
                    </code>
                    <TooltipProvider>
                      <Tooltip>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Este endpoint está ativo por 24 horas e irá armazenar os últimos 50 webhooks recebidos.
                  </p>
                </div>
                
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-base font-medium mb-4">Como usar o Sandbox</h3>
                    <ol className="space-y-3 text-sm">
                      ...
                      <li className="flex gap-2">
                        ...
                        <p>Configure este endpoint no seu sistema ou aplicação</p>
                      </li>
                      <li className="flex gap-2">
                        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
                          3
                        </span>
                        <p>Acione eventos no seu sistema que disparem webhooks</p>
                      </li>
                      <li className="flex gap-2">
                        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
                          4
                    ...
                    <div className="mt-6 p-3 border rounded-md bg-amber-50 border-amber-200 text-sm">
                      <h4 className="font-medium text-amber-800 mb-1">Ambiente seguro</h4>
                      <p className="text-amber-700 text-xs">
                        O sandbox é um ambiente isolado e seguro. Nenhum webhook enviado 
                        para cá será encaminhado para sistemas externos.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="border rounded-md">
                    <div className="flex items-center justify-between p-3 border-b">
                      <h3 className="text-sm font-medium">Webhooks Recebidos</h3>
                    ...
                    <div className="p-8 text-center">
                      ...
                      <p className="text-muted-foreground text-xs mt-1">
                        Configure o URL do sandbox no seu sistema e acione eventos
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                  ...
                  <TabsContent value="javascript" className="mt-2">
                    <div className="relative">
                      ...
                      <Button 
                        ...
                        onClick={() => copyCode(generateCodeSample('javascript'))}
                      >
                ...
                <div className="bg-muted/50 p-4 rounded-md text-sm mt-4 border">
                  ...
                  <ul className="space-y-2 text-sm">
                    ...
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-green-600 mt-0.5" />
                      <p>Retorne 200 OK assim que o webhook for recebido, mesmo que o processamento seja assíncrono</p>
                    </li>
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-green-600 mt-0.5" />
                      <p>Verifique a assinatura do webhook para garantir que os dados são legítimos</p>
                    </li>
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-green-600 mt-0.5" />
                      <p>Implemente tratamento de idempotência para evitar duplicação</p>
                    </li>
                  </ul>
                </div>
              </div>
